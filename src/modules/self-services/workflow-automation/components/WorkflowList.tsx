...
export default function WorkflowList({
  ...
  const getLocalizedCategory = (workflow: WorkflowItem) => {
    if (currentLocale === "ar") {
      return (
        workflow.workflow_category_arb ||
        workflow.workflow_category_eng ||
        "N/A"
      );
    }
    return (
      workflow.workflow_category_eng || workflow.workflow_category_arb || "N/A"
    );
  };
  ...
  const columns: TableColumn<WorkflowItem>[] = [
    ...
    },
    {
      ...
      header:
        t("leaveManagement.workflowAutomation.columns.category") || "Category",
      accessor: (workflow) => (
        <div className="text-sm">{getLocalizedCategory(workflow)}</div>
      ),
    },
    {
      key: "status",
      header:
        t("leaveManagement.workflowAutomation.columns.status") || "Status",
      accessor: (workflow) => (
