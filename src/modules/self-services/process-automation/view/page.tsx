...
export default function ProcessAutomationPage() {
  ...
  const columns: TableColumn<WorkflowType>[] = [
    {
      key: "code",
      header: t("workflow.code") || "Code",
      accessor: (item) => item.workflow_code || "-",
      width: "w-40",
    },
    {
      key: "name",
      header: t("workflow.name") || "Name",
      accessor: (item) => getLocalizedName(item),
    },
    {
      key: "steps",
      header: t("workflow.steps") || "Steps",
      accessor: (item) => item._count?.workflow_type_steps || 0,
    },
  ];

  const getItemId = (item: WorkflowType) => item.workflow_id;
  const getItemDisplayName = (item: WorkflowType) =>
    getLocalizedName(item) || item.workflow_code || String(item.workflow_id);
  ...
  const handleEdit = (item: WorkflowType) => {
    router.push(
      `/leave-management/process-automation/edit/${item.workflow_id}`
    );
  };
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-start">
      <div className="w-full relative">
        <div className="py-4 border-border bg-background/90 p-4">
          <ProcessAutomationHeader
            ...
            onAdd={() =>
              router.push("/leave-management/process-automation/add")
            }
          ...
          <GenericTable
            ...
            onSelectItem={(id) => selectItem(Number(id))}
            onSelectAll={selectAll}
            onEditItem={handleEdit}
            onDeleteItem={(id) => handleDelete(Number(id))}
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setDeleteDialog({ open: false, type: null })
        }
      >
