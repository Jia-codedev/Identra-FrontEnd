...
interface DeptAdminsHeaderProps {
  search: string;
...
export default function DeptAdminsHeader({ search, setSearch, onAdd, selectedCount = 0, onDeleteSelected }: DeptAdminsHeaderProps) {
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div className="flex-1">
          ...
          <p className="text-base md:text-lg text-muted-foreground font-normal mb-2">
            {t('masterData.deptAdmins.description') || ''}
          </p>
          {selectedCount > 0 && (
        ...
        <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-2 items-center">
          <div className={`flex items-center gap-0 bg-card/80 border border-border rounded-xl px-2 py-1 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <Input
              placeholder={t('masterData.deptAdmins.searchPlaceholder') || 'Search'}
              value={search}
            ...
            {selectedCount > 0 && onDeleteSelected ? (
              ...
              <Button
                ...
                variant="default"
              >
                <span className="hidden sm:inline">+ {t('masterData.deptAdmins.add')}</span>
