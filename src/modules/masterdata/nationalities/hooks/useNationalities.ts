...
import { NationalitiesState, INationality } from "../types";
...
export const useNationalities = () => {
  const [state, setState] = useState<NationalitiesState>({
    nationalities: [],
    selected: [],
    search: "",
    page: 1,
  ...
  const {
    ...
    queryKey: ["nationalities", state.search, state.pageSize],
    queryFn: ({ pageParam = 1 }) =>
      nationalitiesApi
        .getNationalities({
          offset: pageParam,
  ...
  const allIds = nationalities
    .filter((n: INationality) => n && n.citizenship_id !== undefined)
    .map((n: INationality) => n.citizenship_id);
  const allChecked =
  ...
  const selectNationality = useCallback((id: number) => {
    setState((prev) => ({
      ...prev,
