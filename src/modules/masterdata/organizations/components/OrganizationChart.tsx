...
export type ChartStyle = 'hierarchical' | 'radial' | 'horizontal' | 'compact' | 'mindmap';
...
const OrganizationNode: React.FC<{ data: OrganizationNodeData }> = ({ data }) => {
    ...
    const getNodeColor = (level: number) => {
        ...
        switch (style) {
            ...
            case 'radial':
                const radialColors = ['#10b981', '#06b6d4', '#8b5cf6', '#f59e0b', '#ef4444', '#64748b'];
                return radialColors[Math.min(level, radialColors.length - 1)];
            case 'compact':
                const compactColors = ['#6366f1', '#8b5cf6', '#ec4899', '#f97316', '#10b981', '#64748b'];
                return compactColors[Math.min(level, compactColors.length - 1)];
            case 'horizontal':
                const horizontalColors = ['#7c3aed', '#2563eb', '#dc2626', '#059669', '#f59e0b', '#64748b'];
                return horizontalColors[Math.min(level, horizontalColors.length - 1)];
            default: // hierarchical
                const colors = ['#1e40af', '#059669', '#dc2626', '#ca8a04', '#6366f1', '#64748b'];
    ...
    const getDisplayName = (org: IOrganizationStructure) => {
        if (isArabic) {
            return org.organization_arb || org.organization_eng || t('common.notAvailable');
        }
        return org.organization_eng || org.organization_arb || t('common.notAvailable');
    };
    ...
    const getOrganizationTypeName = (orgType: any) => {
        if (!orgType) return '';
        if (isArabic) {
            return orgType.organization_type_arb || orgType.organization_type_eng || '';
        }
    ...
    return (
        <div className="relative">
            ...
            <div className={`${getNodeSize()} p-2 shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer bg-white ${getBorderStyle()} ${isArabic ? 'text-right font-arabic' : 'text-left'}`}
                style={{ borderColor: getNodeColor(level) }}>
                <div className="p-1 rounded-t-md" style={{ backgroundColor: `${getNodeColor(level)}28` }}>
                    <div className={`flex items-center gap-1.5 ${isArabic ? 'flex-row-reverse' : ''}`}>
                        <Building2 size={12} style={{ color: getNodeColor(level) }} />
                        <h3 className={`text-[11px] p-0 font-semibold truncate leading-tight ${isArabic ? 'font-arabic' : ''}`}>
                            {getDisplayName(organization)}
                        </h3>
                    </div>
                    {getSecondaryName(organization) && (
                        <p className={`text-[9px] text-muted-foreground truncate mt-0.5 ${isArabic ? 'font-arabic' : ''}`}>
                            {getSecondaryName(organization)}
                        </p>
                    )}
                </div>
                <div className="px-2 py-1 space-y-1">
                    <div className={`flex items-center justify-between ${isArabic ? 'flex-row-reverse' : ''}`}>
                        ...
                        <Badge
                            ...
                            style={{ backgroundColor: `${getNodeColor(level)}20`, color: getNodeColor(level) }}
                        >
                            {t('common.level')}{level + 1}
                        </Badge>
                    </div>

                    {organization.organization_types && (
                        <div className={`flex items-center gap-1 text-[9px] text-muted-foreground ${isArabic ? 'flex-row-reverse' : ''}`}>
                            <Users className="h-2 w-2 flex-shrink-0" />
                            <span className="truncate">{getOrganizationTypeName(organization.organization_types)}</span>
                        </div>
                    )}
                    ...
                    {organization.locations && (
                        <div className={`flex items-center gap-1 text-[9px] text-muted-foreground ${isArabic ? 'flex-row-reverse' : ''}`}>
                            ...
                            <span className="truncate">{getLocationName(organization.locations)}</span>
                        </div>
                    )}
                </div>
            </div>
...
export const OrganizationChart: React.FC<OrganizationChartProps> = ({ 
    ...
    const { nodes: initialNodes, edges: initialEdges } = useMemo(() => {
        ...
        const calculateSubtreeWidth = (org: IOrganizationStructure): number => {
            ...
            const childrenWidth = org.children.reduce((sum, child) =>
        ...
        const processHierarchicalNode = (
            ...
            if (org.children && org.children.length > 0) {
                const totalChildrenWidth = org.children.reduce((sum, child) =>
                    sum + calculateSubtreeWidth(child), 0
                );
                const childSpacing = Math.max(layoutParams.BASE_HORIZONTAL_SPACING || 280,
                ...
                org.children.forEach((child, _index) => {
                    ...
                    const childCenterOffset = (childSubtreeWidth - 1) * childSpacing / 2;
                    ...
                    edges.push(edge);
                    processHierarchicalNode(child, level + 1, childX, childSpacing * childSubtreeWidth, 0);
        ...
        const processHorizontalNode = (
            ...
            if (org.children && org.children.length > 0) {
                const totalChildrenHeight = org.children.reduce((sum, child) =>
                    sum + calculateSubtreeWidth(child), 0
                );
                const childSpacing = Math.max(layoutParams.BASE_VERTICAL_SPACING || 120,
                ...
                org.children.forEach((child, _index) => {
                    ...
                    const childCenterOffset = (childSubtreeHeight - 1) * childSpacing / 2;
                    ...
                    edges.push(edge);
                    processHorizontalNode(child, level + 1, childY, childSpacing * childSubtreeHeight, 0);
        ...
        const processCompactNode = (
            ...
            if (org.children && org.children.length > 0) {
                ...
                const childSpacing = Math.max(layoutParams.BASE_HORIZONTAL_SPACING || 200,
                    availableWidth / Math.max(totalChildrenWidth, 1));
                const startX = nodeX - (totalChildrenWidth - 1) * childSpacing / 2;
                ...
                org.children.forEach((child, _index) => {
                    ...
                    edges.push(edge);
                    processCompactNode(child, level + 1, childX, childSpacing * childSubtreeWidth, 0);
                    currentChildX += childSubtreeWidth * childSpacing;
                });
            }
        ...
        const processMindMapNode = (
            ...
            if (org.children && org.children.length > 0) {
                ...
                const angleStep = org.children.length > 1 ? angleSpread / (org.children.length - 1) : angleSpread / 2;
                const startAngle = angle - angleSpread / 2;

                org.children.forEach((child, index) => {
                    const childAngle = org.children.length === 1 ? angle : startAngle + (angleStep * index);
                    ...
                    edges.push(edge);
                    processMindMapNode(child, level + 1, childAngle, childDistance, false);
                });
            }
        };
        ...
        switch (style) {
            case 'radial':
                if (data.length === 1) {
                    ...
                    data.forEach((rootOrg, index) => {
                        ...
                        processRadialNode(rootOrg, 0, angle, layoutParams.BASE_RADIUS || 150);
                    });
                }
                break;
            ...
            case 'compact':
                if (data.length === 1) {
                    ...
                    const totalRootWidth = data.reduce((sum, org) => sum + calculateSubtreeWidth(org), 0);
                    const rootSpacing = Math.max(layoutParams.BASE_HORIZONTAL_SPACING || 200, 800 / totalRootWidth);
                    ...
                    data.forEach((rootOrg) => {
                        ...
                        processCompactNode(rootOrg, 0, rootX, rootSpacing * rootSubtreeWidth, 0);
            ...
            case 'mindmap':
                if (data.length === 1) {
                    ...
                    data.forEach((rootOrg, index) => {
                        const angle = angleStep * index;
                        processMindMapNode(rootOrg, 0, angle, layoutParams.BRANCH_LENGTH || 250, false);
                    });
                }
                break;
            ...
            default:
                if (data.length === 1) {
                    ...
                    const totalRootWidth = data.reduce((sum, org) => sum + calculateSubtreeWidth(org), 0);
                    const rootSpacing = Math.max((layoutParams.BASE_HORIZONTAL_SPACING || 280) * 2, 1000 / totalRootWidth);
                    let currentRootX = -(totalRootWidth - 1) * rootSpacing / 2;
                    ...
                    data.forEach((rootOrg) => {
                        ...
                        processHierarchicalNode(rootOrg, 0, rootX, rootSpacing * rootSubtreeWidth, 0);
                        currentRootX += rootSubtreeWidth * rootSpacing;
                    });
                }
                break;
        ...
        console.log(`Generated ${nodes.length} nodes and ${edges.length} edges for ${style} style`);
        return { nodes, edges };
    ...
    return (
        <div className={`w-full h-[650px] ${isArabic ? 'rtl' : 'ltr'}`} dir={isArabic ? 'rtl' : 'ltr'}>
            <ReactFlow
            ...
            >
                ...
                />
                <Background
                    variant={style === 'mindmap' ? BackgroundVariant.Dots : BackgroundVariant.Lines}
                    gap={style === 'compact' ? 12 : 20}
                    size={style === 'compact' ? 0.5 : 1}
                    color={style === 'mindmap' ? '#e5e7eb' : '#f3f4f6'}
                />
            </ReactFlow>
        </div>
    );
