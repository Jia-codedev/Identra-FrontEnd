...
export const OrganizationTypeModal: React.FC<OrganizationTypeModalProps> = ({
  ...
  const [formData, setFormData] = useState<OrganizationTypeFormFields>(initialForm);

  useEffect(() => {
    if (organizationType && mode === 'edit') {
      setFormData({
        organization_type_eng: organizationType.organization_type_eng === "" ? undefined : organizationType.organization_type_eng,
        organization_type_arb: organizationType.organization_type_arb === "" ? undefined : organizationType.organization_type_arb,
  ...
  const handleInputChange = (field: keyof OrganizationTypeFormFields, value: string | number) => {
    setFormData(prev => ({ 
      ...prev, 
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4 dark:bg-white/5 bg-black/5 rounded-xl p-2">

            {
              isRTL ? (
                <div>
                  <Label htmlFor="organization_type_arb" className="text-sm font-medium text-right w-full">
                    {t('masterData.organizationTypes.organizationTypeNameArabic')}
                  </Label>
                  <Input
                    id="organization_type_arb"
                    value={formData.organization_type_arb}
                    onChange={(e) => handleInputChange('organization_type_arb', e.target.value)}
                    placeholder={t('masterData.organizationTypes.enterOrganizationTypeNameArabic')}
                ...
                <div>
                  <Label htmlFor="organization_type_eng" className="text-sm font-medium">
                    {t('masterData.organizationTypes.organizationTypeNameEnglish')} *
                  </Label>
                  <Input
                    id="organization_type_eng"
                    value={formData.organization_type_eng}
                    onChange={(e) => handleInputChange('organization_type_eng', e.target.value)}
                    placeholder={t('masterData.organizationTypes.enterOrganizationTypeNameEnglish')}
                    required
                    className="mt-1"
                  />
                </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleInputChange('org_type_level', e.target.value)}
