...
interface OrganizationTypesHeaderProps {
  search: string;
  onSearchChange: (value: string) => void;
  onAddOrganizationType: () => void;
  selectedCount: number;
  onDeleteSelected?: () => void;
}

export const OrganizationTypesHeader: React.FC<OrganizationTypesHeaderProps> = ({
  search,
  onSearchChange,
  onAddOrganizationType,
  selectedCount,
  onDeleteSelected,
}) => {
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        ...
        <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-2 items-center">
          <div className={`flex items-center gap-0 bg-card/80 border border-border rounded-xl px-2 py-1 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <Input
              placeholder={t('masterData.organizationTypes.searchPlaceholder')}
              value={search}
