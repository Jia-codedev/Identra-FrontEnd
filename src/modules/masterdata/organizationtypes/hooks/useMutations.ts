...
export function useOrganizationTypeMutations() {
  ...
  const createOrganizationTypeMutation = useMutation({
    mutationFn: async ({
      ...
      organizationTypeData: IOrganizationType;
      onClose: () => void;
      search: string;
      pageSize: number;
    }) => {
      ...
      const data = await organizationTypesApi.addOrganizationType(
    ...
    onSuccess: (data) => {
      if (!data) return;
      toast.success(
        t("masterData.organizationTypes.toast.success.organizationTypeAdded")
      );
    ...
    onError: (error) => {
      toast.error(
        t("masterData.organizationTypes.toast.error.organizationTypeAddError")
      );
  ...
  const updateOrganizationTypeMutation = useMutation({
    mutationFn: async ({
      ...
      const data = await organizationTypesApi.updateOrganizationType(
        id,
        organizationTypeData
      );
      if (data.status !== 200) {
    ...
    onSuccess: (data) => {
      if (!data) return;
      toast.success(
        t("masterData.organizationTypes.toast.success.organizationTypeUpdated")
      );
      queryClient.setQueryData(
        ["organizationtypes", data.search ?? "", data.pageSize ?? 5],
        (oldData: any) => {
    ...
    onError: (error) => {
      toast.error(
        t(
          "masterData.organizationTypes.toast.error.organizationTypeUpdateError"
        )
      );
  ...
  const deleteOrganizationTypeMutation = useMutation({
    ...
    onSuccess: ({ id }) => {
      toast.success(
        t("masterData.organizationTypes.toast.success.organizationTypeDeleted")
      );
    ...
    onError: (error) => {
      toast.error(
        t(
          "masterData.organizationTypes.toast.error.organizationTypeDeleteError"
        )
      );
  ...
  const deleteOrganizationTypesMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        t(
          "masterData.organizationTypes.toast.success.organizationTypeMultipleDelete"
        )
      );
      queryClient.setQueriesData(
        { queryKey: ["organizationtypes"] },
    ...
    onError: (error) => {
      toast.error(
        t(
          "masterData.organizationTypes.toast.error.organizationTypeMultipleDeleteError"
        )
      );
