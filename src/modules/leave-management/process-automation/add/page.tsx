...
export default function AddWorkflowPage() {
  ...
  React.useEffect(() => {
    ...
    rolesApi.getRoles({ offset: 1, limit: 10 }).then((res) => {
      ...
      const opts = arr.map((r: any) => {
        const label = r.role_name_eng || r.role_name || r.role_name_arb || String(r.role_id);
  ...
  const addStep = () =>
    setSteps((s) => {
      ...
      // ensure only last step is final
  ...
  const removeStep = (index: number) =>
    setSteps((s) => {
      ...
      // Re-sequence and ensure final is last
      return next.map((st, i) => ({ ...st, step_order: i + 1, is_final_step: i === next.length - 1 }));
    });

  const updateStep = (index: number, field: string, value: any) =>
    setSteps((s) => {
      // If toggling is_final to true, move this step to the end
  ...
  const handleSubmit = async () => {
    ...
    try {
      // created_id and last_updated_id are required by backend schemas; using 1 as placeholder
      const payload = {
        workflow_code: code,
      ...
      // Add steps if any (filter out empty steps)
      ...
      if (validSteps.length > 0) {
        for (const step of validSteps) {
          const payloadStep = {
            ...
            role_id: step.role_id !== null && step.role_id !== undefined ? Number(step.role_id) : undefined,
  ...
  return (
    <div className="p-6 space-y-6">
      <h2 className="text-2xl font-semibold">{t('workflow.add') || 'Add Workflow'}</h2>

      {/* Top card: Workflow metadata */}
