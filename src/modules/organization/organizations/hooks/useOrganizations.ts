...
import { OrganizationsState, IOrganization } from "../types";
...
export const useOrganizations = () => {
  ...
  const {
    ...
    queryKey: ["organizations", state.search, state.pageSize],
    queryFn: ({ pageParam = 1 }) =>
      organizationsApi
        .getOrganizations({
          offset: pageParam,
  ...
  const allIds = organizations
    .filter((org: IOrganization) => org && org.organization_id !== undefined)
    .map((org: IOrganization) => org.organization_id);
  const allChecked =
