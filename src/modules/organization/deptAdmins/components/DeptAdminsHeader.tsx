...
export default function DeptAdminsHeader({
  search,
  setSearch,
  onAdd,
  selectedCount = 0,
  onDeleteSelected,
}: DeptAdminsHeaderProps) {
  const { t } = useTranslations();
  const { isRTL } = useLanguage();

  return (
    <div className="sticky top-0 z-10 bg-background/80 rounded-t-3xl  py-8 ">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        ...
        <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-2 items-center">
          ...
          >
            ...
            <Input
              placeholder={
                t("masterData.deptAdmins.searchPlaceholder") || "Search"
              }
              value={search}
