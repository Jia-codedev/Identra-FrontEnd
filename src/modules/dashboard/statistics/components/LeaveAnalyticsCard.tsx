...
function LeaveAnalyticsCard() {
  return (
    <div className="shadow-card rounded-[10px] bg-background p-2 border border-border/60">
      ...
      <ChartContainer config={chartConfig} className="relative left-[-30px]">
        <BarChart accessibilityLayer data={chartData}>
          ...
          <Bar dataKey="leaves" stackId="a" fill="var(--primary)" opacity={0.5} radius={[0, 0, 2, 2]} />
          <Bar dataKey="absent" stackId="a" fill="var(--primary)" opacity={0.8} radius={[2, 2, 0, 0]} />
        </BarChart>
      </ChartContainer>
    </div> 
  )
