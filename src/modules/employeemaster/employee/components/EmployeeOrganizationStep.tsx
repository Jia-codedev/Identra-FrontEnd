...
export const EmployeeOrganizationStep: React.FC<EmployeeStepProps> = ({
  ...
  // Async dropdown states for employee type, designation, location
  const [employeeTypeOptions, setEmployeeTypeOptions] = useState<{ label: string; value: number }[]>([]);
  ...
  const loadManagers = useCallback(async (searchTerm: string) => {
    ...
    try {
      ...
      // If there's a search term, fetch matching managers
      ...
      if (formData.manager_id) {
        const hasSelectedManager = managerList.some(m => m.value === formData.manager_id);
  ...
  const loadEmployeeTypes = useCallback(async (searchTerm: string) => {
    ...
    try {
      const response = await employeeTypeApi.getEmployeeTypes({ offset: 1, limit: 20, search: searchTerm });
      if (response.data?.success && Array.isArray(response.data.data)) {
        setEmployeeTypeOptions(response.data.data.map((item: any) => ({
  ...
  const loadDesignations = useCallback(async (searchTerm: string) => {
    ...
    try {
      const response = await designationsApi.getDesignations({ offset: 1, limit: 20, search: searchTerm });
      if (response.data?.success && Array.isArray(response.data.data)) {
        setDesignationOptions(response.data.data.map((item: any) => ({
  ...
  const loadLocations = useCallback(async (searchTerm: string) => {
    ...
    try {
      const response = await locationsApi.getLocations({ offset: 1, limit: 20, name: searchTerm });
      if (response.data?.success && Array.isArray(response.data.data)) {
        setLocationOptions(response.data.data.map((item: any) => ({
  ...
  useEffect(() => {
    if (formData.employee_type_id && employeeTypeOptions.length === 0) {
      const loadCurrentEmployeeType = async () => {
        try {
          const response = await employeeTypeApi.getEmployeeTypeById(formData.employee_type_id!);
          if (response.data?.success && response.data.data) {
  ...
  useEffect(() => {
    if (formData.designation_id && designationOptions.length === 0) {
      const loadCurrentDesignation = async () => {
        try {
          const response = await designationsApi.getDesignationById(formData.designation_id!);
          if (response.data?.success && response.data.data) {
            const item = response.data.data;
            setDesignationOptions([{
              label: item.designation_eng || item.name,
  ...
  useEffect(() => {
    if (formData.location_id && locationOptions.length === 0) {
      const loadCurrentLocation = async () => {
        try {
          const response = await locationsApi.getLocationById(formData.location_id!);
          if (response.data?.success && response.data.data) {
            const item = response.data.data;
            setLocationOptions([{
              label: item.location_eng || item.name,
  ...
  // Initial load for dropdowns (default options)
  useEffect(() => {
    loadEmployeeTypes("");
  }, [loadEmployeeTypes]);
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Combobox
            ...
            onValueChange={(value) => onInputChange("organization_id", value ? parseInt(value.toString()) : null)}
            placeholder={isLoadingData ? t("common.loading") : t("employeeMaster.employee.selectOrganization")}
        ...
        <div>
          ...
          <Combobox
            ...
            onValueChange={(value) => onInputChange("designation_id", value ? parseInt(value.toString()) : null)}
            onSearch={(query) => setDesignationSearchTerm(query)}
            placeholder={isLoadingData ? t("common.loading") : t("employeeMaster.employee.selectDesignation")}
            disabled={isLoadingData}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Combobox
            ...
            onValueChange={(value) => onInputChange("employee_type_id", value ? parseInt(value.toString()) : null)}
            onSearch={(query) => setEmployeeTypeSearchTerm(query)}
            placeholder={isLoadingData ? t("common.loading") : t("employeeMaster.employee.selectEmployeeType")}
            disabled={isLoadingData}
        ...
        <div>
          ...
          <Combobox
            ...
            onValueChange={(value) => onInputChange("grade_id", value ? parseInt(value.toString()) : null)}
            placeholder={isLoadingData ? t("common.loading") : t("employeeMaster.employee.selectGrade")}
            disabled={isLoadingData}
            className="mt-1"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Combobox
            ...
            onValueChange={(value) => onInputChange("location_id", value ? parseInt(value.toString()) : null)}
            onSearch={(query) => setLocationSearchTerm(query)}
            placeholder={isLoadingData ? t("common.loading") : t("employeeMaster.employee.selectLocation")}
            disabled={isLoadingData}
