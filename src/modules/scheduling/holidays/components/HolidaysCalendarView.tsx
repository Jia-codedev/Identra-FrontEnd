...
const HolidaysCalendarView: React.FC<HolidaysCalendarViewProps> = ({
  holidays,
}) => {
                ...
                <div className="flex items-start justify-between">
                  <div
                    className={`text-sm font-semibold ${
                      inMonth ? "text-card-foreground" : "text-muted-foreground"
                    }`}
                  >
                    {format(day, "d")}
                  </div>
                  {items.length > 0 && (
                    <div className="text-[11px] text-muted-foreground">
                      {items.length}{" "}
                      {items.length === 1 ? "holiday" : "holidays"}
                    </div>
                  )}
                </div>
                    ...
                    <Popover key={h.holiday_id}>
                      ...
                      <PopoverContent>
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {h.from_date
                              ? format(parseISO(h.from_date), "MMM dd")
                              : ""}
                            {h.to_date && h.to_date !== h.from_date
                              ? ` - ${format(parseISO(h.to_date), "MMM dd")}`
                              : ""}
                          </div>
                        </div>
                      </PopoverContent>
      ...
      <div className="mt-6">
        ...
        <div className="flex flex-wrap gap-2">
          {holidays.slice(0, 8).map((h) => (
            <div
              key={h.holiday_id}
              className="flex items-center gap-2 bg-card/50 p-2 rounded-md"
            >
              <div className="text-sm font-medium">
                {isRTL ? h.holiday_arb : h.holiday_eng}
              </div>
              <div className="text-xs text-muted-foreground">
                {format(parseISO(h.from_date), "MMM dd")}
              </div>
