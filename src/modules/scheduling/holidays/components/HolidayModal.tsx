...
export const HolidayModal: React.FC<HolidayModalProps> = ({
  ...
  const holidaySchema = useMemo(() => z.object({
    holiday_eng: isRTL ? z.string().optional() : z.string().min(1, 'English name is required'),
    holiday_arb: isRTL ? z.string().min(1, 'Arabic name is required') : z.string().optional(),
  ...
  // Format date for input field (YYYY-MM-DD format) - Currently unused
  ...
  // Format datetime for API (ISO string) - Currently unused
  // const formatDateTimeForAPI = (dateString: string, isEndDate: boolean = false) => {
  //   try {
  //     const date = new Date(dateString);
  //     if (isEndDate) {
  ...
  const onSubmit = (data: DynamicHolidayFormValues) => {
    ...
    const submittedName = isRTL ? (data.holiday_arb || '') : (data.holiday_eng || '');
    
    const formattedData: CreateHolidayRequest = {
      from_date: data.from_date.toISOString(),
      to_date: data.to_date.toISOString(),
      // Always provide both languages for API compatibility
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-w-[95vw] max-h-[95vh] overflow-y-auto p-0">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6 p-4 sm:p-6">
            <div className="grid grid-cols-1 gap-4">
              {/* Holiday Name - Language Specific */}
              {isRTL ? (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm sm:text-base">{t('common.nameArb')}</FormLabel>
                      <FormControl>
                        <Input
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm sm:text-base">{t('common.nameEng')}</FormLabel>
                      <FormControl>
                        <Input
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel className="text-sm sm:text-base">{t('common.fromDate')}</FormLabel>
                    <Popover>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel className="text-sm sm:text-base">{t('common.toDate')}</FormLabel>
                    <Popover>
                      <PopoverTrigger asChild>
                        <FormControl>
                          <Button
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-sm sm:text-base">{t('common.remarks')}</FormLabel>
                  <FormControl>
                    <Textarea
