...
const buildingsApi = {
  ...
  list: async (params: ListBuildingsRequest = {}): Promise<ApiResponse<Building[]>> => {
    ...
    if (params.building_code) {
      filteredBuildings = filteredBuildings.filter(building =>
        building.building_code.toLowerCase().includes(params.building_code!.toLowerCase())
      );
    }

    if (params.building_status !== undefined) {
      filteredBuildings = filteredBuildings.filter(building =>
        building.building_status === params.building_status
      );
    }
    ...
    if (params.delete_flag !== undefined) {
      filteredBuildings = filteredBuildings.filter(building =>
        building.delete_flag === params.delete_flag
      );
    }
  ...
  update: async (id: number, data: UpdateBuildingRequest): Promise<ApiResponse<Building>> => {
    ...
    const buildingIndex = mockBuildings.findIndex(b => b.id === id && !b.delete_flag);
    if (buildingIndex === -1) {
      throw new Error("Building not found");
    }
  ...
  delete: async (id: number): Promise<ApiResponse<void>> => {
    ...
    mockBuildings[buildingIndex].last_updated_date = new Date().toISOString().split('T')[0];
