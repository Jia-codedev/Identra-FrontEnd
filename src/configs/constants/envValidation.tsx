...
class FrontendSettings {
  ...
  constructor() {
    ...
    if (typeof envValue === "string" && envValue.trim() !== "") {
      console.log(`✅ Environment variable NEXT_PUBLIC_IDENTRA_BE_URI loaded successfully: ${envValue}`);
      this.apiBaseUrl = envValue;
    } else {
      console.log(`⚠️ Environment variable NEXT_PUBLIC_IDENTRA_BE_URI not found, using fallback: http://localhost:8000`);
  ...
  private validateEnvVar(name: string, fallback?: string): string {
    ...
    if (typeof value === "string" && value.trim() !== "") {
      console.log(`✅ Environment variable ${name} loaded successfully: ${value}`);
      return value;
    }
    
    if (fallback !== undefined) {
      console.log(`⚠️ Environment variable ${name} not found, using fallback: ${fallback}`);
      return fallback;
    }
